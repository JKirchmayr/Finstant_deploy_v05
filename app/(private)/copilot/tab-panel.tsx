...
const TabPanelComponent = () => {
  ...
  const closeTabPanel = (id: number) => {
    ...
    if (newTabList.length > 0) {
      const newActiveTab = currentIndex > 0 ? newTabList[currentIndex - 1] : newTabList[0];
  ...
  return (
    ...
    >
          ...
          {tabList.map((tab) => {
            return (
              ...
              >
                ...
                {tab.type === "investors" && (
                  <CircleDollarSignIcon className="-ms-0.5 me-1.5 opacity-60" size={16} />
                )}
                {tab.type === "investor-profile" && (
                  <UserCircle2Icon className="-ms-0.5 me-1.5 opacity-60" size={16} />
                )}
                {tab.type === "company-profile" && (
                  <FactoryIcon className="-ms-0.5 me-1.5 opacity-60" size={16} />
                )}
      ...
      <TabsContent value={activeTabId} className="flex-1 h-full overflow-auto bg-white noscroll">
